...
export default function NFTCardScreen({ route }) {
  ...
  const combinedTransformStyle = {
    transform: [...flipToBackStyle.transform, ...shakeStyle.transform],
  };

  return (
    <ImageBackground
      source={require('../assets/images/NFTBackground.png')}
      style={styles.screen}
      deferRender={true}
    >
      <HeaderBack />
      ...
      {isFlipped && (
        <Text style={styles.touchText}>
          꽃피 NFT 카드<Text style={styles.touchText2}>를 터치해보세요</Text>
        </Text>
      )}
    </ImageBackground>
  );
